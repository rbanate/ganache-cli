...
var Ganache = require('rlb-ganache-core');
var pkg = require('./package.json');
var corepkg = require('./node_modules/rlb-ganache-core/package.json');
var URL = require('url');
var Web3 = require('web3');
var web3 = new Web3(); // Used only for its BigNumber library.
var fs = require('fs');

var parser = yargs().option('unlock', {
  type: 'string',
  alias: 'u'
});
...
console.log(
  'Ganache CLI v' + pkg.version + ' (rlb-ganache-core: ' + corepkg.version + ')'
);
...
server.listen(options.port, options.hostname, function(err, result) {
  ...
  addresses.forEach(function(address, index) {
    console.log(
      '(' + index + ') ' + accounts[address].secretKey.toString('hex')
    );
  });
  ...
  if (options.fork) {
    ...
    console.log(
      'Block:       ' +
        web3.toBigNumber(state.blockchain.fork_block_number).toString(10)
    );
